...
} = require("@picovoice/picovoice-node/dist/errors");
const { PvRecorder } = require("@picovoice/pvrecorder-node");
const {
  BuiltinKeyword,
  getBuiltinKeywordPath,
} = require("@picovoice/porcupine-node");
const inferenceCallback = require("./intents");
...
async function copilot() {
  ...
  if (
    ...
    console.error(
      "--endpointDurationSec must be a number in the range [0.5, 5.0]",
    );
    return;
  }
  ...
  const keywordCallback = require("./wakeword")(
    friendlyKeywordName,
    contextFilename,
  );
